...
var bullet_life: int = 3
# 子弹当前行进时间
var cur_time: float = 0.0
var next_point: Vector2 = Vector2.ZERO
var cur_tower_pos: Vector2 = Vector2.ZERO
...
func _ready() -> void:
	...
	if is_instance_valid(target):
		cur_enemy_pos = target.global_position + Vector2(randf_range(-10, 10), randf_range(-10, 10))
...
func fire(_pos: Vector2, _tar: Area2D) -> void:
	position = _pos + Vector2(randf_range(-10, 10), randf_range(-10, 10))
	target = _tar
